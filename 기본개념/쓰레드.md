...
## 쓰레드 동기화
멀티 쓰레드 환경에서, 여러 쓰레드가 자원을 공유할 수 있기에 서로의 작업에 영향을 줄 수 있다. A 쓰레드가 작업하던 도중 실행대기 상태에 돌입하고 B 쓰레드가 공유 자원을 변경하고 실행대기 상태에 돌입하고 A 쓰레드가 다시 작업을 할 때 공유 자원이 변경되었기에 의도치 않은 결과가 발생할 수 있다. 이를 방지하기 위해선, 한 쓰레드가 작업 중일 때 다른 쓰레드의 방해받지 않아야한다. 이와 관련된 개념이 '임계 영역'과 '락(lock)'이다. 공유 데이터를 사용하는 영역을 임계 영역으로 지정해두고 해당 영역에 진입할 때 다른 쓰레드가 진입하지 못하도록 락을 걸어두고 작업이 끝나면 락을 반환해 다른 쓰레드가 안전하게 다시 작업을 수행할 수 있도록 하는 것이다. 이와 같이 작업 중인 쓰레드가 다른 쓰레드에게 방해받지 않도록 하는 것을 쓰레드의 동기화라고 한다.
